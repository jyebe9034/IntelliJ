...
public class KISA_SHA256 {
    ...
    /**
     * SHA-256 SEED 암호화 (단방향)
     * @param source
     * @return 암호화된 문자열
     * @throws IllegalArgumentException 문자열이 비어있을 경우
     */
    public static String encrypt(String source) throws IllegalArgumentException {
        if(StringUtils.isEmpty(source)) throw new IllegalArgumentException("문자열이 비었음");

        byte pbCipher[]   = new byte[32];
        SHA256_Encrpyt( source.getBytes(), source.length(), pbCipher );

        String result = "";
        for (int i=0; i<32; i++) {
            String s = Integer.toHexString(0xff&pbCipher[i]);
            if(s.length() < 2) s = "0"+s;
            result += s;
        }
        return result;
	...
	// Encryption
        ...
        System.out.println("result : " + encrypt("791007"));


/*
		SHA256_Encrpyt( "791007".getBytes(), "791007".length(), pbCipher );
